...
- Optional:
  ...
  - CREATE_VIEWS ('true'/'false', defaults to 'false')
...
CREATE_VIEWS = os.getenv("CREATE_VIEWS", "false").strip().lower() in {"1", "true", "yes", "y"}
...
CENTER_LIAISONS_PATH = os.getenv("CENTER_LIAISONS_PATH", "center_liasons.yaml")
...
def main() -> None:
    ...
    phase2_centers = all_centers[safe_contains(all_centers["name"], r"(htan2|HTAN2_)")]
    phase2_centers = phase2_centers[~safe_contains(phase2_centers["name"], r"(TEST|DEV)")].copy()
